...
function setup_vendor() {
    ...
    export SOSP_ROOT="$3"
    if [ ! -d "$SOSP_ROOT" ]; then
        echo "\$SOSP_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SOSP_ROOT/$OUTDIR" ]; then
        mkdir -p "$SOSP_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SOSP_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SOSP_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SOSP_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SOSP_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR SOSP-OS\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR SOSP-OS\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR SOSP-OS\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 SOSP-OS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR SOSP-OS\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR SOSP-OS\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR SOSP-OS\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR SOSP-OS\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SOSP_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$SOSP_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SOSP_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SOSP_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$SOSP_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SOSP_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SOSP_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$SOSP_ROOT"/vendor/sosp/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SOSP_ROOT"/"$OUTDIR"/radio
